...
import vaeengine
from vaeengine.tools import copy_cfg, list_cfg, train
from vaeengine.tools.analysis_tools import show_vae
from vaeengine.tools.model_converters import publish_model2diffusers
from vaeengine.tools.preprocess import bucket_ids
...
CLI_HELP_MSG = \
    f"""
    Arguments received: {['vaeengine'] + sys.argv[1:]!s}. vaeengine commands use the following syntax:

        vaeengine MODE MODE_ARGS ARGS

        Where   MODE (required) is one of {MODES}
                MODE_ARG (optional) is the argument for specific mode
                ARGS (optional) are the arguments for specific command

    Some usages for vaeengine commands: (See more by using -h for specific command!)

        1. List all predefined configs:
            vaeengine list-cfg
        2. Copy a predefined config to a given path:
            vaeengine copy-cfg $CONFIG $SAVE_FILE
        3-1. Fine-tune by a single GPU:
            vaeengine train $CONFIG
        3-2. Fine-tune by multiple GPUs:
            NPROC_PER_NODE=$GPU_NUM vaeengine train $CONFIG
        4-1. Convert the pth model to HuggingFace's model:
            vaeengine convert pth_to_hf $CONFIG $PATH_TO_PTH_MODEL $SAVE_PATH_TO_HF_MODEL
        5-1. Preprocess bucket ids:
            vaeengine preprocess bucket_ids
        6-1. Show VAE result for a given image path:
            vaeengine analyze show_vae $IMAGE_PATH
    ...
    GitHub: https://github.com/okotaku/vaeengine
    """  # noqa: E501
...
special = {
    ...
    "version": lambda: print_log(vaeengine.__version__, "current"),
}
special = {
    **special,
    **{f"-{k[0]}": v
